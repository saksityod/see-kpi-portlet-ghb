$(document).ready(() => {
  ...
  const methods = {
    ...
    fetchDropdownData: async () => {
      ...
      console.log(unstructuredYear);

      if (unstructuredYear) {
        state.years = unstructuredYear.map(item => {
          return {
            id: item.current_appraisal_year,
            name: item.current_appraisal_year
          };
        });
      }

      let unstructuredPeriod = await $.ajax({
        url: `${apiUrl}/public/cds_result/year_list`,
        type: 'GET',
        headers: {
          Authorization: tokenID.token
        }
      ...
      let unstructuredOrgLevel = await $.ajax({
        url: `${apiUrl}/public/cds_result/org_list_v2`,
        type: 'GET',
        headers: {
          Authorization: tokenID.token
        }
    ...
    applySearchListeners: (yearSelector, periodSelector, orgSelector, empSelector, searchForm) => {
      if (yearSelector) {
  ...
  const state = {
    ...
    set orgs(orgs) {
      this._orgs = orgs;
      methods.renderDropdownItems($('#orgLevelSelectItems'), orgs);
    },
    ...
    set emps(emps) {
      this._emps = emps;
      methods.renderDropdownItems($('#empLevelSelectItems'), emps);
    },
