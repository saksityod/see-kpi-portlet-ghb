...
class DashboardController extends Controller
{
	...
	{	
		...
		$qinput = [];
		$query = "
			select c.org_id, if(c.appraisal_type_id=1,e.emp_name,d.org_name) name, a.item_id, b.item_name, ifnull(a.target_value,0) target_value, ifnull(a.actual_value,0) actual_value, a.weight_percent, a.etl_dttm, c.result_threshold_group_id,
		...
		foreach ($items as $i) {
			...
			if ($i->weight_percent > $max_value) {
				$max_value = $i->weight_percent;
			}
		...
		return response()->json(['dataset' => $dataset, 'header' => $header, 'max_value' => $max_value, 'categories' => array($cat), 'trendlines' => array($trendlines)]);	
	}
	...
	public function kpi_overall(Request $request)
	{
		...
		$max_value = 0;
		foreach ($items as $i) {
			if ($i->actual_value > $max_value) {
				$max_value = $i->actual_value;
			}
		...
		return response()->json(['dataset' => $dataset, 'header' => $header, 'max_value' => $max_value]);
	}
